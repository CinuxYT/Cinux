    ...
    try:
        ...
        try:
            if req.json()["status_code"] == 0:
                ...
                if DebugMode:
                    print(Colorate.Horizontal(Colors.green_to_white, f"CinuxYT Sent Share: {SentTotalSentShares} ({impr_id})"))
                else:
                    print(Colorate.Horizontal(Colors.green_to_white, f"CinuxYT Sent Share: {SentTotalSentShares} ({impr_id})"))
...
if __name__ == "__main__":
    Clear()
    itemID = Write.Input("CinuxYT Video Link > ", Colors.red_to_purple, interval=0.0001)
    amount = Write.Input("CinuxYT Adet (0=inf) > ", Colors.red_to_purple, interval=0.0001)
    NThread = Write.Input("CinuxYT Gönderim Hızı  > ", Colors.red_to_purple, interval=0.0001)
