    ...
    try:
        ...
        try:
            if req.json()["status_code"] == 0:
                ...
                TotalSendedShare += 1
                if DebugMode:
                    print(Colorate.Horizontal(Colors.red_to_white, f"CinuxYT Paylaşıldı: {TotalSendedShare} ({impr_id})"))
                else:
                    print(Colorate.Horizontal(Colors.red_to_white, f"CinuxYT Paylaşıldı: {TotalSendedShare} ({impr_id})"))
                    Title(f"Hız :{str(active_count() - 1)} / Hit :{TotalSendedShare} / Hatalı :{TotalFailedReq}")
            else:
                pass
        except:
            TotalFailedReq += 1
...
if __name__ == "__main__":
    Clear()
    itemID = Write.Input("CinuxYT Video Link > ", Colors.blue_to_white, interval=0.0001)
    amount = Write.Input("Adet [0-sınırsız] > ", Colors.blue_to_white, interval=0.0001)
    NThread = Write.Input("Hız > ", Colors.blue_to_white, interval=0.0001)

    if Title("Proy Scrapper X-Proxy by ctcanime"):
